...
public abstract class BaseBean implements Serializable, Cloneable {
     ...
     *         需要解析的class类型
     */
    public static void setResponseObject(Result result, Class<? extends BaseBean> cls) {
        if (Model.checkSuccess(result)) {
            try {
                BaseBean bean = newInstance(cls, (String) result.getData());
                result.setData(bean);
            } catch (JSONException e) {
                e.printStackTrace();
                Context context = BaseApplication.getInstance().getApplicationContext();
                result.setCode(Integer.parseInt(context.getString(R.string.exception_local_json_code)));
                result.setMsg(context.getString(R.string.exception_local_json_message));
            }
        }
    }
